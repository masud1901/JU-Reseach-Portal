    ...
    try {
      ...
      if (!publicationsError && publicationsData) {
        ...
        if (!allPubsError && allPubs) {
          const citationCount = allPubs.reduce((sum, pub) => sum + (pub.citation_count || 0), 0);
  ...
  async function fetchStudentData(studentId: string) {
    try {
      ...
      if (!authorError && authorData) {
        const publications = authorData.map(a => a.publications);
        setRecentPublications(publications);
        
        // Calculate stats
        const citationCount = publications.reduce((sum, pub) => sum + (pub.citation_count || 0), 0);
    ...
    return (
      <div className="container mx-auto py-8 px-4">
        ...
        <div className="bg-muted/30 p-6 rounded-lg mb-8">
          ...
          <p className="mb-6">You don't have a profile yet. Create a profile to connect with researchers and students.</p>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle>Professor Profile</CardTitle>
                <CardDescription>For faculty members and researchers</CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4">
                  Create a professor profile to showcase your research, connect with students, and collaborate with other researchers.
                </p>
              </CardContent>
              <CardFooter>
                <Button onClick={() => navigate("/create-professor-profile")} className="w-full">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>For students and research assistants</CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4">
                  Create a student profile to find research mentors, showcase your interests, and connect with professors.
                </p>
              </CardContent>
              <CardFooter>
                <Button onClick={() => navigate("/create-student-profile")} className="w-full">
    ...
    <div className="container mx-auto py-8 px-4">
      <Tabs defaultValue={professorProfile ? "professor" : "student"}>
        <div className="flex justify-between items-center mb-8">
          ...
          <TabsList>
            {professorProfile && <TabsTrigger value="professor">Professor View</TabsTrigger>}
            {studentProfile && <TabsTrigger value="student">Student View</TabsTrigger>}
          </TabsList>
        </div>
          ...
          <TabsContent value="professor" className="space-y-8">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <Avatar className="h-16 w-16 border-2 border-primary/10">
                      <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${professorProfile.name}`} />
                      <AvatarFallback>
                        {professorProfile.name.split(" ").map(n => n[0]).join("").toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      ...
                      <CardDescription>{professorProfile.department}</CardDescription>
                  ...
                  <Button variant="outline" size="sm" onClick={() => navigate(`/professors/${professorProfile.id}`)}>
                    View Profile
                  </Button>
                </div>
              </CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-muted/30 p-4 rounded-lg flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{stats.publicationCount}</p>
                    </div>
                  </div>
                  <div className="bg-muted/30 p-4 rounded-lg flex items-center gap-3">
                    <div className="bg-primary/10 p-2 rounded-full">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{stats.citationCount}</p>
                    </div>
                  </div>
                  <div className="bg-muted/30 p-4 rounded-lg flex items-center gap-3">
                    <div className="bg-primary/10 p-2 rounded-full">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold">{stats.connectionCount}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your most recent research publications</CardDescription>
              </CardHeader>
              <CardContent>
                {recentPublications.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <Button 
                      variant="link" 
                      onClick={() => navigate(`/professors/${professorProfile.id}`)}
                      className="mt-2"
                    >
                    ...
                    {recentPublications.map((pub) => (
                      <div key={pub.id} className="border-b pb-4 last:border-0">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {pub.journal_name || pub.publisher}, {pub.publication_year}
                        </p>
                        <div className="flex items-center gap-2 mt-1">
            ...
            <Card>
              ...
              <CardContent>
                {connectionRequests.filter(req => req.to_user_id === user.id).length === 0 ? (
                  <div className="text-center py-6">
          ...
          <TabsContent value="student" className="space-y-8">
            ...
            <Card>
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <Button variant="outline" size="sm" onClick={() => navigate(`/students/${studentProfile.id}`)}>
                    View Profile
                  </Button>
                </div>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Research you've contributed to</CardDescription>
              </CardHeader>
              <CardContent>
                {recentPublications.length === 0 ? (
                  <div className="text-center py-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your connection requests to professors</CardDescription>
              </CardHeader>
              <CardContent>
                {connectionRequests.filter(req => req.from_user_id === user.id).length === 0 ? (
                  <div className="text-center py-6">
                    <p className="text-muted-foreground">No pending connection requests</p>
                    <Button 
                      variant="link" 
                    ...
                    {connectionRequests
                      ...
                      .map((request) => (
                        <div key={request.id} className="flex items-center justify-between border-b pb-4 last:border-0">
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="font-medium">Connection request pending</p>
